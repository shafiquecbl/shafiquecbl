<div align="center">

<img src="https://cdn.jsdelivr.net/gh/shafiquecbl/shafiquecbl@main/assets/avatar.webp" width="116" alt="Muhammad Shafique" />

# Muhammad Shafique

### Flutter products, engineered to move.

Flutter Team Lead building fast, reliable mobile products across security, delivery, AI, health, education, publishing, and fintech.

[**View portfolio ↗**](https://shafique.dev) &nbsp;&nbsp;·&nbsp;&nbsp; [LinkedIn](https://www.linkedin.com/in/mr-shafique) &nbsp;&nbsp;·&nbsp;&nbsp; [Email](mailto:shafiquecbl@gmail.com) &nbsp;&nbsp;·&nbsp;&nbsp; [Résumé](https://shafique.dev/Shafique-Resume.pdf)

</div>

<br>

<table width="100%">
<tr align="center">
<td width="25%"><h3>6+</h3><sub>YEARS BUILDING</sub></td>
<td width="25%"><h3>30+</h3><sub>PRODUCTS SHIPPED</sub></td>
<td width="25%"><h3>20+</h3><sub>LAUNCHES DELIVERED</sub></td>
<td width="25%"><h3>2</h3><sub>MOBILE PLATFORMS</sub></td>
</tr>
</table>

## 01 / About

<table>
<tr>
<td width="62%" valign="top">

### I build for the parts that cannot feel unfinished.

I turn complex product ideas into polished Flutter experiences—from the first architecture decision through native integrations, subscriptions, analytics, CI/CD, and store release.

My work lives where **mobile craft meets platform depth**: interfaces that feel considered, systems that remain dependable, and products that are maintainable long after launch.

</td>
<td width="38%" valign="top">

**PRACTICE**<br>
Mobile product engineering

**FOCUS**<br>
Architecture → release

**CURRENTLY**<br>
Flutter Team Lead at Dcodax

**BASED**<br>
Islamabad, Pakistan

</td>
</tr>
</table>

## 02 / Selected work

<p>Production products, available in the wild. Every image below is from the shipped application.</p>

<table>
<tr>
<td width="50%" valign="top">

<a href="https://apps.apple.com/us/app/sdrd/id6747887489"><img src="https://cdn.jsdelivr.net/gh/shafiquecbl/shafiquecbl@main/assets/projects/sdrd.webp" width="100%" alt="SDRD mobile delivery application" /></a>

### SDRD

**Multi-module delivery ecosystem**

Food, grocery, shop, and parcel delivery built around fast ordering, discovery, maps, and daily commerce flows.

`Flutter` `Delivery` `Maps` `Commerce`

[App Store ↗](https://apps.apple.com/us/app/sdrd/id6747887489) &nbsp;·&nbsp; [Google Play ↗](https://play.google.com/store/apps/details?id=com.sadrad.sdrd)

</td>
<td width="50%" valign="top">

<a href="https://apps.apple.com/us/app/sdrd-partner/id6747889277"><img src="https://cdn.jsdelivr.net/gh/shafiquecbl/shafiquecbl@main/assets/projects/sdrd-partner.webp" width="100%" alt="SDRD Partner merchant operations application" /></a>

### SDRD Partner

**Merchant operations**

A merchant command center for live orders, catalog controls, store availability, wallet withdrawals, earnings, and performance reporting.

`Flutter` `Orders` `Wallet` `Tablet`

[App Store ↗](https://apps.apple.com/us/app/sdrd-partner/id6747889277) &nbsp;·&nbsp; [Google Play ↗](https://play.google.com/store/apps/details?id=com.sadrad.store&hl=en_CA)

</td>
</tr>
<tr>
<td width="50%" valign="top">

<a href="https://apps.apple.com/us/app/guardx-vpn-vpn-proxy-master/id6753163496"><img src="https://cdn.jsdelivr.net/gh/shafiquecbl/shafiquecbl@main/assets/projects/guardx.webp" width="100%" alt="GuardX VPN mobile application" /></a>

### GuardX VPN

**VPN + mobile security**

A privacy-focused VPN with OpenVPN, WireGuard, and Xray protocols, plus threat protection, split tunneling, subscriptions, and speed testing.

`Flutter` `WireGuard` `Xray` `RevenueCat`

[App Store ↗](https://apps.apple.com/us/app/guardx-vpn-vpn-proxy-master/id6753163496)

</td>
<td width="50%" valign="top">

<a href="https://apps.apple.com/us/app/vynox-vpn-proxy-master/id6737821640"><img src="https://cdn.jsdelivr.net/gh/shafiquecbl/shafiquecbl@main/assets/projects/vynox.webp" width="100%" alt="Vynox VPN mobile application" /></a>

### Vynox VPN

**Consumer VPN**

A one-tap privacy product for private browsing, streaming, gaming, global server selection, auto-connect, subscriptions, and ad blocking.

`Flutter` `VPN` `Privacy` `RevenueCat`

[App Store ↗](https://apps.apple.com/us/app/vynox-vpn-proxy-master/id6737821640) &nbsp;·&nbsp; [Google Play ↗](https://play.google.com/store/apps/details?id=vynox.secure.vpn.proxy.master&hl=en)

</td>
</tr>
<tr>
<td width="50%" valign="top">

<a href="https://apps.apple.com/us/app/ai-photo-generator-pixart/id6737462382"><img src="https://cdn.jsdelivr.net/gh/shafiquecbl/shafiquecbl@main/assets/projects/pixart.webp" width="100%" alt="PixArt AI mobile application" /></a>

### PixArt AI

**Generative AI studio**

An image generation and editing workspace for creating, previewing, refining, and saving visual concepts inside one focused mobile product.

`Flutter` `Generative AI` `Image Editing` `IAP`

[App Store ↗](https://apps.apple.com/us/app/ai-photo-generator-pixart/id6737462382) &nbsp;·&nbsp; [Google Play ↗](https://play.google.com/store/apps/details?id=pixart.aiart.generator)

</td>
<td width="50%" valign="top">

<a href="https://apps.apple.com/us/app/f1-race-programme-2024/id6478093369"><img src="https://cdn.jsdelivr.net/gh/shafiquecbl/shafiquecbl@main/assets/projects/f1-race-programme.webp" width="100%" alt="F1 Race Programme mobile application" /></a>

### F1 Race Programme

**Official race guide**

A 30+ year publication reimagined with news, standings, schedules, circuit maps, galleries, and championship updates.

`Flutter` `Publishing` `Schedules` `Media`

[App Store ↗](https://apps.apple.com/us/app/f1-race-programme-2024/id6478093369)

</td>
</tr>
<tr>
<td width="50%" valign="top">

<a href="https://apps.apple.com/us/app/flow-glow/id6738675614"><img src="https://cdn.jsdelivr.net/gh/shafiquecbl/shafiquecbl@main/assets/projects/flow-and-glow.webp" width="100%" alt="Flow and Glow health tracking application" /></a>

### Flow & Glow

**Health and wellness tracker**

Sensitive daily-use flows for cycles, symptoms, weight, temperature, educational content, and pregnancy tracking.

`Flutter` `Health` `Tracking` `Privacy`

[App Store ↗](https://apps.apple.com/us/app/flow-glow/id6738675614) &nbsp;·&nbsp; [Google Play ↗](https://play.google.com/store/apps/details?id=app.flow.glow.periods.pregnancy.tracker)

</td>
<td width="50%" valign="top">

<a href="https://play.google.com/store/apps/details?id=com.moonport.moonportapp"><img src="https://cdn.jsdelivr.net/gh/shafiquecbl/shafiquecbl@main/assets/projects/moonport.webp" width="100%" alt="Moonport crypto calculator application" /></a>

### Moonport

**Crypto scenario calculator**

Fast projections across 10,000+ coins, future balances, market caps, and price targets—turning volatile data into comparable decision views.

`Flutter` `Crypto` `Market Data` `Fintech`

[Google Play ↗](https://play.google.com/store/apps/details?id=com.moonport.moonportapp)

</td>
</tr>
<tr>
<td width="50%" valign="top">

<a href="https://apps.apple.com/us/app/prophets-stories-pbut/id6755478899"><img src="https://cdn.jsdelivr.net/gh/shafiquecbl/shafiquecbl@main/assets/projects/prophets-stories.webp" width="100%" alt="Prophet's Stories educational mobile application" /></a>

### Prophet's Stories

**Islamic education**

Illustrated lessons, audio narration, English translations, and offline access shaped into a calm learning experience for families and students.

`Flutter` `Education` `Audio` `Offline`

[App Store ↗](https://apps.apple.com/us/app/prophets-stories-pbut/id6755478899) &nbsp;·&nbsp; [Google Play ↗](https://play.google.com/store/apps/details?id=com.wisecodex.prophetstories)

</td>
<td width="50%" valign="top">

<a href="https://apps.apple.com/us/app/ahadees-al-rasool/id6749274573"><img src="https://cdn.jsdelivr.net/gh/shafiquecbl/shafiquecbl@main/assets/projects/ahadees-al-rasool.webp" width="100%" alt="Ahadees Al-Rasool educational application" /></a>

### Ahadees Al-Rasool

**Hadith learning**

Authentic Hadith learning with Arabic recitations, English translations, searchable chapters, bookmarks, illustrations, and offline audio.

`Flutter` `Hadith` `Audio` `Offline`

[App Store ↗](https://apps.apple.com/us/app/ahadees-al-rasool/id6749274573) &nbsp;·&nbsp; [Google Play ↗](https://play.google.com/store/apps/details?id=com.wisecodex.ahadeesalrasool)

</td>
</tr>
</table>

<div align="center">

[**Explore every product case study →**](https://shafique.dev/#work)

</div>

## 03 / Core stack

<p align="center">
  <img src="https://cdn.simpleicons.org/flutter/54C5F8" height="38" alt="Flutter" title="Flutter" />&nbsp;&nbsp;&nbsp;
  <img src="https://cdn.simpleicons.org/dart/0175C2" height="38" alt="Dart" title="Dart" />&nbsp;&nbsp;&nbsp;
  <img src="https://cdn.simpleicons.org/kotlin/7F52FF" height="38" alt="Kotlin" title="Kotlin" />&nbsp;&nbsp;&nbsp;
  <img src="https://cdn.simpleicons.org/swift/F05138" height="38" alt="Swift" title="Swift" />&nbsp;&nbsp;&nbsp;
  <img src="https://cdn.simpleicons.org/firebase/FFCA28" height="38" alt="Firebase" title="Firebase" />&nbsp;&nbsp;&nbsp;
  <img src="https://cdn.simpleicons.org/revenuecat/F25A5A" height="38" alt="RevenueCat" title="RevenueCat" />&nbsp;&nbsp;&nbsp;
  <img src="https://cdn.simpleicons.org/googlemaps/4285F4" height="38" alt="Google Maps" title="Google Maps" />&nbsp;&nbsp;&nbsp;
  <img src="https://cdn.simpleicons.org/githubactions/2088FF" height="38" alt="GitHub Actions" title="GitHub Actions" />&nbsp;&nbsp;&nbsp;
  <img src="https://cdn.simpleicons.org/xcode/147EFB" height="38" alt="Xcode" title="Xcode" />&nbsp;&nbsp;&nbsp;
  <img src="https://cdn.simpleicons.org/androidstudio/3DDC84" height="38" alt="Android Studio" title="Android Studio" />&nbsp;&nbsp;&nbsp;
  <img src="https://cdn.simpleicons.org/git/F05032" height="38" alt="Git" title="Git" />
</p>

<table>
<tr>
<td width="25%" valign="top"><strong>Mobile craft</strong><br><sub>Responsive UI · Motion · State management · Store builds</sub></td>
<td width="25%" valign="top"><strong>Product systems</strong><br><sub>Subscriptions · Analytics · Maps · Payments · REST APIs</sub></td>
<td width="25%" valign="top"><strong>Native depth</strong><br><sub>VPN · Network extensions · Kotlin · Swift · Permissions</sub></td>
<td width="25%" valign="top"><strong>Release & scale</strong><br><sub>CI/CD · Xcode Cloud · GitHub Actions · AWS · Monitoring</sub></td>
</tr>
</table>

## 04 / Experience

<table>
<tr>
<td width="22%" valign="top"><sub>DEC 2021 — PRESENT</sub></td>
<td width="78%" valign="top"><strong>Flutter Team Lead · Dcodax Technologies</strong><br>Leading architecture and delivery across VPN, AI image generation, health, food delivery, publishing, education, and SaaS products.</td>
</tr>
<tr>
<td width="22%" valign="top"><sub>JAN 2021 — NOV 2021</sub></td>
<td width="78%" valign="top"><strong>Flutter Developer · Secure Hops</strong><br>Delivered client mobile products with production-ready Flutter flows and REST API integrations.</td>
</tr>
</table>

## 05 / Open source

<table>
<tr>
<td width="70%" valign="top">

### [flutter_v2ray_plus ↗](https://github.com/shafiquecbl/flutter_v2ray_plus)

VLESS and VMESS proxy/VPN support for Android and iOS, powered by the V2Ray/Xray core. Includes routing, share-link parsing, delay testing, status tracking, and extended configuration.

`Flutter` `Dart` `Android` `iOS` `VLESS` `VMESS` `Xray`

</td>
<td width="30%" valign="middle" align="center">

**OPEN-SOURCE FLUTTER PLUGIN**

[pub.dev ↗](https://pub.dev/packages/flutter_v2ray_plus)<br>
[Source code ↗](https://github.com/shafiquecbl/flutter_v2ray_plus)

</td>
</tr>
</table>

## 06 / GitHub activity

<a href="https://github.com/shafiquecbl?tab=overview&from=2025-09-01&to=2026-09-09">
<picture>
  <source media="(prefers-color-scheme: dark)" srcset="https://ghchart.rshah.org/54c5f8/shafiquecbl">
  <source media="(prefers-color-scheme: light)" srcset="https://ghchart.rshah.org/0969da/shafiquecbl">
  <img src="https://ghchart.rshah.org/54c5f8/shafiquecbl" width="100%" alt="Muhammad Shafique's GitHub contribution history" />
</picture>
</a>

<table width="100%">
<tr>
<td width="50%" align="center" valign="top">
<picture>
  <source media="(prefers-color-scheme: dark)" srcset="https://github-readme-stats-eight-theta.vercel.app/api?username=shafiquecbl&show_icons=true&hide_border=true&bg_color=00000000&text_color=c9d1d9&title_color=54c5f8&icon_color=54c5f8">
  <source media="(prefers-color-scheme: light)" srcset="https://github-readme-stats-eight-theta.vercel.app/api?username=shafiquecbl&show_icons=true&hide_border=true&bg_color=00000000&text_color=24292f&title_color=0969da&icon_color=0969da">
  <img src="https://github-readme-stats-eight-theta.vercel.app/api?username=shafiquecbl&show_icons=true&hide_border=true&bg_color=00000000&text_color=c9d1d9&title_color=54c5f8&icon_color=54c5f8" width="100%" alt="Muhammad Shafique's GitHub statistics" />
</picture>
</td>
<td width="50%" align="center" valign="top">
<picture>
  <source media="(prefers-color-scheme: dark)" srcset="https://github-readme-stats-eight-theta.vercel.app/api/top-langs/?username=shafiquecbl&layout=compact&hide_border=true&bg_color=00000000&text_color=c9d1d9&title_color=54c5f8">
  <source media="(prefers-color-scheme: light)" srcset="https://github-readme-stats-eight-theta.vercel.app/api/top-langs/?username=shafiquecbl&layout=compact&hide_border=true&bg_color=00000000&text_color=24292f&title_color=0969da">
  <img src="https://github-readme-stats-eight-theta.vercel.app/api/top-langs/?username=shafiquecbl&layout=compact&hide_border=true&bg_color=00000000&text_color=c9d1d9&title_color=54c5f8" width="100%" alt="Muhammad Shafique's top programming languages" />
</picture>
</td>
</tr>
</table>

---

<div align="center">

## Bring me the product that needs to feel finished.

[**Start a project →**](mailto:shafiquecbl@gmail.com)

[shafique.dev](https://shafique.dev) &nbsp;·&nbsp; [LinkedIn](https://www.linkedin.com/in/mr-shafique) &nbsp;·&nbsp; Islamabad, Pakistan

</div>
